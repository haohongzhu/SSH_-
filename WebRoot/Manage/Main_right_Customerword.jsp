...
<!--  	<s:form method="post" action="adminQueryAllAction" namesapce="/">-->
<!--  		<s:submit value="显示所有用户" />-->
<!--  	</s:form>-->
	<s:action name="adminQueryAllAction" executeResult="false"/>
  	<s:form action="adminQueryByNameAction" method="post" namesapce="/" theme="simple">
		<span>姓名：</span><s:textfield name="realName"/>
		<s:submit value="查询"/>
	</s:form>
	<div class="layui-form">
		<table class="layui-table" id="table_showWordMsg" >
			<tr>
		...
		<s:iterator value="#word.customerWords" var="customerWords">
		<tr align="center" >
			<td><s:property value="#word.realName"/></td>	
			<td><s:property value="#word.userTelephone"/></td>
			<td><s:property value="#customerWords"/></td>
			<td>
		...
		[<a href="adminQueryAllAction?pageNo=1" class="yema">首页</a>]
			<c:choose>
				<c:when test="${currentPage>1}">
					[<a href="adminQueryAllAction?pageNo=${currentPage-1}" class="yema">上一页</a>]
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${currentPage<totalPage}">
					[<a href="adminQueryAllAction?pageNo=${currentPage+1}" class="yema">下一页</a>]
				</c:when>
			</c:choose>
	[<a href="adminQueryAllAction?pageNo=${totalPage}" class="yema">尾页</a>]
	（当前第${currentPage}/${totalPage}页）
	</div>
  	<!--<table id="table_showWordMsg" >
		<tr>
			<th><span class="right_th">用户姓名</span></th>
			<th><span class="right_th">用户电话号码</span></th>
			<th><span class="right_th">用户留言</span></th>
			<th><span class="right_th">操作</span></th>
		</tr>
		<s:iterator value="customerWords" status="st" var="word">
		<s:iterator value="#word.customerWords" var="customerWords">
		<tr align="center" >
